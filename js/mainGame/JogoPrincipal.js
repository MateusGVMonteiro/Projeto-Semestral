...
function MarioGame() {
  ...
  var translatedDist; //distância traduzida (rolagem lateral) enquanto Mario se move para a direita
  var centerPos; //posição central do viewPort, tela visível
  var marioInGround;
  ...
  var instructionTick = 0; //mostrando contador de instruções
  var that = this;
    ...
    canvas.addEventListener('touchstart', function(e) {
      ...
      for (var i = 0; i < touches.length; i++) {
        ...
        if (touches[i].pageX > 640 && touches[i].pageX <= 1080) {
          //em eventos de toque, a mesma área atua como sprint e bullet key
  ...
  this.startGame = function() {
    ...
    that.updateMario();
    that.wallCollision();
    marioInGround = mario.grounded; //para uso com deslizamento de bandeira
  };

  this.showInstructions = function() {
    gameUI.writeText('Controles: Setas para direcao, shift para correr', 30, 30);
    gameUI.writeText('Dica: Pular enquanto corre faz voce pular mais alto, utilizando o shift', 30, 60);
  };
  ...
  this.checkElementMarioCollision = function(element, row, column) {
    ...
    if (collisionDirection == 'l' || collisionDirection == 'r') {
      ...
      if (element.type != 5) {
        //só se não for mastro de bandeira
        mario.grounded = true;
        mario.jumping = false;
      }
      ...
      if (element.type == 2) {
        ...
        map[row][column] = 4; //define uma caixa inútil depois que a moeda aparece
        //som quando o bloco de moedas é atingido
        gameSound.play('coin');
      }
    }
  };
  ...
  this.checkElementPowerUpCollision = function(element) {
    for (var i = 0; i < powerUps.length; i++) {
      ...
      if (collisionDirection == 'l' || collisionDirection == 'r') {
        powerUps[i].velX *= -1; //mudar de direção se houver colisão com qualquer elemento do sidr
      } else if (collisionDirection == 'b') {
  ...
  this.checkElementBulletCollision = function(element) {
    for (var i = 0; i < bullets.length; i++) {
      ...
      if (collisionDirection == 'b') {
        //se a colisão ocorrer na parte inferior da bala, ela será aterrada, para que possa ser rebatida
  ...
  this.checkEnemyMarioCollision = function() {
    for (var i = 0; i < goombas.length; i++) {
      if (!mario.invulnerable && goombas[i].state != 'dead' && goombas[i].state != 'deadFromBullet') {
        ...
        if (collWithMario == 't') {
          //mate goombas se a colisão for de cima
          ...
          if (mario.type == 'big') {
            ...
            //mate Mario se a colisão ocorrer quando ele for pequeno
            that.pauseGame();
    ...
    for (var i = 0; i < goombas.length; i++) {
      for (var j = 0; j < bullets.length; j++) {
        if (goombas[i] && goombas[i].state != 'dead') {
          //verifique se há colisão apenas se os goombas existirem e não estiverem mortos
  ...
  this.wallCollision = function() {
    ...
    //para solo (terreno da janela de visualização)
    if (mario.y >= height) {
      that.pauseGame();
  ...
  //controlando Mario com eventos importantes
  this.updateMario = function() {
    ...
    if (keys[38] || keys[32]) {
      ...
      if (!mario.jumping && mario.grounded) {
        ...
        if (mario.frame == 0 || mario.frame == 1) {
          mario.frame = 3; //salto direito
        } else if (mario.frame == 8 || mario.frame == 9) {
          mario.frame = 2; //salto para a esquerda
        }

        //som quando Mario pula
        gameSound.play('jump');
      }
    }
    ...
    if (keys[39]) {
      //seta direita
      that.checkMarioPos(); //se Mario for para o centro da tela, role o mapa para o lado
    ...
    if (mario.grounded) {
      ...
      if (mario.frame == 3) {
        mario.frame = 0; //olhando para a direita
      } else if (mario.frame == 2) {
        mario.frame = 8; //olhando para a esquerda
      }
    }

    //mudar a posição do Mário
    mario.velX *= friction;
    mario.velY += gravity;
  ...
  this.checkMarioPos = function() {
    ...
    //rolagem lateral enquanto Mario chega ao centro do viewPort
  ...
  this.levelFinish = function(collisionDirection) {
    //O jogo termina quando Mario desliza o mastro da bandeira e colide com o chão
  ...
  this.gameOver = function() {
    ...
    gameUI.writeText('Game Over meu Carioca', centerPos - 150, height - 300);
    gameUI.writeText('Obrigado por jogar', centerPos - 122, height / 2);
  };
